...
- `SCHEDULER_LOG_LEVEL` (optional; `debug|info|warn|error`, default `info`)
- `SCHEDULER_DEFAULT_SESSION_ID` (optional; used by `scheduler_create_task_quick` when `session_id` omitted)
- `SCHEDULER_DEFAULT_WORKSPACE_ID` (optional; used by `scheduler_create_task_quick`, fallback `opencode-prod`)
- `SCHEDULER_DEFAULT_API_KEY` (optional; used by `scheduler_create_task_quick` when `api_key` omitted)
- `SCHEDULER_DEFAULT_EXECUTE_ENDPOINT` (optional; default `http://127.0.0.1:9090/api/scheduler/execute`)
- `SCHEDULER_DEFAULT_CALLBACK_URL` (optional; default `http://127.0.0.1:9090/api/scheduler/callback`)
- `SCHEDULER_DEFAULT_TIMEZONE` (optional; default `Asia/Shanghai`)
...
`scheduler_create_task_quick` can omit `workspace_id` and `api_key` if corresponding `SCHEDULER_DEFAULT_*` env vars are set.
