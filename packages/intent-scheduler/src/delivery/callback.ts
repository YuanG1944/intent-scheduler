  ...
  async deliver(
    ...
    for (let i = 0; i < retryDelays.length; i += 1) {
      ...
      try {
        ...
        if (response.ok) {
          logger.info("delivery.callback.success", {
        ...
        logger.warn("delivery.callback.non_2xx", {
          run_id: payload.run_id,
          attempt: i + 1,
          status_code: response.status,
        });
        ...
        logger.error("delivery.callback.error", {
          run_id: payload.run_id,
          attempt: i + 1,
    ...
    logger.error("delivery.callback.exhausted", {
      run_id: payload.run_id,
